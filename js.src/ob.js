	...
	(function($){
	//hteeml
	$['Þ'] = function (arrSrc,options,target) {

	var 
		target = target&&target!==$&&target.size?target:false;
		
		if(!window[G_DATA] && G_DATA === License){
			window[G_DATA]={
				FLIPS:{},
				NUMBER:0,
				BOOKS:[],
				...
				buildNextBook:function(){
					...
					if(book){
						...
						if(book.target){
							var oldbookName = $(book.target).attr('Þbook');
						...
						var BOOK_CONSTRUCTOR = $.extend( true, {}, Þ3D );
	...
	var Þ3D = {
		...
		init:function(images_src,options,target){
			...
			ÞEdit.init($.extend(this.options,{
		...
		initWithData:function(FRDATA){
			...
			var fn = {
				...
				//var lgs = VN_LANGUAGES;
				//hteeml ru-vi
				var language = {'en':0,'vi':1};
				//hteeml - insert iframe
			...
			this.CFG = {
				...
				//108,117,111,110,103,104,117,117,118,110
				//[79,78,69,66,79,79,75,51,68,84,72,69,66,69,83,84]
			...
			this.AJAX_LOADER = '//w.áq.vn/img/box.gif';
		...
		addStyle:function(){
			...
			styles += zoomLayer+' .btn_title{font:12px Roboto;color:black;padding:5px;white-space:nowrap;}\n';		
			...
			styles += zoomTitleSpread+' p{font:14px Roboto;color:black;line-height:140%;margin:0px;}\n';
			styles += zoomTitleSpread+' p span{font:bold 14px Roboto;color:gray;margin-right:8px;}\n';								
			
			styles += saveMenu+' h1{font:16px Roboto;color:white;}\n';
			styles += saveMenu+' p{font:16px Roboto;color:white;}\n';
			styles += saveMenu+' a{font:16px Roboto;color:white;}\n';
			...
			styles += [
				...
				'font:bold 20px Roboto;color:white;}\n'	
			].join('');
			...
			styles += bookHelpLayer + ' .hlpLeftSide p span[name=click_to_open] a{font:21px Roboto;color:white;}';
			styles += bookHelpLayer + ' .hlpLeftSide p span[name=use_mousewheel]{font:12px Roboto;color:#d4d4d4;}';
			styles += bookHelpLayer + ' .hlpLeftSide p span[name=help_arrow]{width:55px;height:26px;margin:10px 0px 0px auto;opacity:0.8;}';		
			styles += bookHelpLayer + ' .hlpRightSide p span[name=goto_start] a{font:18px Roboto;color:white;}';		

			styles += bookHelpLayer + ' p.middleSize {padding:10px 10px 10px 0px;}';
			styles += bookHelpLayer + ' p.middleSize span{margin-top:5px;}';		
			styles += bookHelpLayer + ' .hlpLeftSide p.middleSize span[name=click_to_open] a{font:16px Roboto;}';
			styles += bookHelpLayer + ' .hlpRightSide p.middleSize span[name=goto_start] a{font:14px Roboto;}';		
			...
			styles += bookIconsPanel+' '+bookSpreadTitle+' p{font:14px Roboto;color:#aaaaaa;line-height:140%;margin:0px;}\n';
			styles += bookIconsPanel+' '+bookSpreadTitle+' span{font:bold 14px Roboto;color:#ffffff;margin-right:8px;}\n';
			styles += bookIconsPanel+'.light '+bookSpreadTitle+' p{color:#000000;}\n';
			styles += bookIconsPanel+'.light '+bookSpreadTitle+' span{color:#888888;}\n';
			styles += bookIconsPanel+' .btn_title{font:12px Roboto;color:#ffffff;padding:5px;white-space:nowrap;}\n';
		...
		buildIconsPanel:function(){
			...
			if(sizeMode === 'tiny'){		
				...
				$book3d_icons.append($leftIconSet.css({floatfloat:'left'}));
				...
				$book3d_icons.append($rightIconSet.css({floatfloat:'right'}));
				this.$gButtonsTitle = $panel.find('.btn_title');
		...
		pageNumbersCreate:function(sizeMode){		
			var 
				...
				$pn = $([
					'<div id="' + id + '" ' + light,
					'style="display:table-cell;vertical-align:middle;width:'+width+'px;font:'+fontSize+'px HL;">',
					'<span style=""></span></div>'
					].join(''));
			return $pn;
		},
		...
		showZoomContent:function(firstCoord){
			...
			var fn = {
				buttonsInsert:function(){
					...
					$zoomButtons.append($rightIconSet.css({floatfloat:'right',margin:'8px 8px 0px 0px'}));
				},
		...
		createGlHlpLayer:function(){
			...
			var fn = {
				PlainTextured:function(options){
					...
					var side = H.L.FrontSide;
					var skin = options.skin;
					var plainMesh = new H.L.Mesh(
						   new H.L.PlaneGeometry( size.width, size.height ),
						   new H.L.MeshBasicMaterial({map:H.L.ImageUtils.loadTexture( transparent ), transparent:true, side:side })
						);
					H.L.ImageUtils.loadTexture(src,undefined,function(texture){
				...
				if(!_this.ZOOM_MODE){
					...
					if(_this.ANIMATION_FLAG){
						...
						if(_this.GLOSSY && _this.GL_LINK_TO_START){
							//hteeml - new albums
							_this.exit();
							//var _bok = document.getElementById('instagram').getAttribute("data-next") + '\/mubla\/3\/moc.rugmi.ipa\/\/';
							
							var _bok = "https://api.imgur.com/3/album/kQXVT";
						
						alert(_bok.split("").reverse().join(""))
						
							$.ajax({
								method: "GET",
								url: _bok, //shkHa kaCMk
								headers: {
								...
								//set next albums
								//document.getElementById('instagram').setAttribute("data-next",result.data.description);
								
								$.Þ(imgArray,{language:'vi', bgLight:'#56998c',bgDark:'#d97f6f',slope:1, border:30});
							}).error(function () {
								//hteeml
		...
		buildAndPreloadSheets:function(){
			...
			var fnGL = {
				PlainTextured:function(options){
					...
					var side = options.backSide ? H.L.BackSide:H.L.FrontSide;
					var plainMesh = new H.L.Mesh(
						   new H.L.PlaneGeometry( options.size[0], options.size[1] ),
						   new H.L.MeshBasicMaterial({map:H.L.ImageUtils.loadTexture( white ), side:side })
						);
					H.L.ImageUtils.loadTexture(src,undefined,function(texture){
					   plainMesh.material.map = texture;
						options.onReady && options.onReady(plainMesh);
		...
		buildResizedImage:function(bookSize,scale,imgWithParam,isSelfShadow){
			...
			//hteeml
			var str = this.getString([169,32,124,32,76,73,77,73,84,69,68,0,76,73,71,72,84,0,86,69,82,83,73,79,78]);
					...
					if(_this.GLOSSY){
						if(gl_sheet && gl_sheet.mesh){
							...
							$.each(gl_edges,function(i){
								...
								H.L.ImageUtils.loadTexture(sheetImages[i].src,undefined,function(texture){
